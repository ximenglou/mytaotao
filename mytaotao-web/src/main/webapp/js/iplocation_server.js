	...
	if(pageConfig.product.specialAttrs&&pageConfig.product.specialAttrs.length>0){
		...
		for(var i=0,j=pageConfig.product.specialAttrs.length;i<j;i++){
			...
			if(pageConfig.product.specialAttrs[i].indexOf("isWeChatStock")>-1){
				...
				if(array.length == 2 && Number(array[1])>0){
		...
		$.ajax({
			...
			success: function(r) {
				if(r){
					...
					if(json.startedFlag+""=="1"&&Number(json.remainTime)>0){$("#red-discount").show();var seconds = Math.floor(json.remainTime/1000 );flashPurchase(seconds);}
				}			
			}
		})
	}
...
function getDeliveCash(r){
    ...
    if(r){
        ...
        else if(r.dtype == 1&&Number(r.dcash)>0&&Number(r.ordermin)>0){
...
function reClick2(type2, pwid, sku, num) {
    var reWidsClubCookies = eval('(' + getCookie(reCookieName) + ')');
    if (reWidsClubCookies == null || reWidsClubCookies == '') reWidsClubCookies = {};
...
function getProvinceStockCallback(result,choosetype) {
    ...
    if (result.stock) {     
        ...
        if(result.stock.D&&result.stock.D.prompt){
            ...
            if (proarray[0]&&Number(proarray[0])>0){
...
function cnp(r){
    ...
    try{
        if(Number(r[0].p)>0&&Number(r[0].m)>0){
            var n = Number(r[0].p)/Number(r[0].m);
            n = Number(n).toFixed(2)<n?parseInt(n*100)+1:parseInt(n*100);
            n = Number(n/10).toFixed(1);
...
function getStockInfoByArea(ipLoc){//获取地区库存
    ...
    if (useAreaPrice&&ipLoc.length>2&&Number(ipLoc[2])>0){
        pricePageLoad = true;
	...
	if(skuIds.length>0){
		$.ajax({
				...
				success:function(r){
					if(r&&r.length>0){
						for (var i=0,j=r.length;i<j;i++){
							$("#"+r[i].id+" .hl_red").html(Number(r[i].p)>0?("￥"+r[i].p):"暂无报价");
						}
					}
				}
			});
...
function getImgFullPath(skuid,src,type){
    if(src&&src.toUpperCase().indexOf("HTTP:")==-1){
        src = "http://img1"+(Number(skuid))%5+".360buyimg.com/n"+type+"/"+src;
    }
    return src;
}
(function(){
    ...
    if(window.itemShopJsona&&itemShopJsona.searchLink){
        var shopSearchHtml='<div id="sp-search" class="m m2" clstag="shangpin|keycount|product|pop-03">'
                    ...
                    +'<div class="mc">'
                        ...
                        +'<p class="sp-form-item2"><label for="sp-price">价&#x3000;格：</label><span><input type="text" id="sp-price" onkeyup="changeSpPrice(\'sp-price\');" onkeydown="if(event.keyCode==13){pageConfig.searchClick(1);}"/> 到 <input type="text" id="sp-price1" onkeyup="changeSpPrice(\'sp-price1\');" onkeydown="if(event.keyCode==13){pageConfig.searchClick(1);}"/></span></p>'
